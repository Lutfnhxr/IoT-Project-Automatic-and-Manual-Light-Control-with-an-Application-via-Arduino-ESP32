...
void syncFirebase(String mode) {
  ...
  // Ini yang akan mengisi folder /logs di Firebase Console Anda
...
void setup() {
  ...
  // Tunggu sinkronisasi waktu NTP
  time_t now = time(nullptr);
  while (now < 1000000) { delay(300); now = time(nullptr); }

  // Ambil setting jadwal dari Firebase
...
void loop() {
  ...
  // Cek Perintah dari Aplikasi (App Control)
